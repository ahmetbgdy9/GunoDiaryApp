...
class SifremiUnuttum : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val textView = findViewById<TextView>(R.id.sifirlaEposta)//girisdeki giriş butonunu buluyoruz
        textView.startAnimation(anim)//asagidan yukarı kaydiran animasyon

        val button1 = findViewById<Button>(R.id.pinUygula)//girisdeki giriş butonunu buluyoruz
        button1.startAnimation(anim)//asagidan yukarı kaydiran animasyon


        button1.setOnClickListener {
            var emailsifirla = findViewById<TextView>(R.id.sifirlaEposta).text.toString().trim()
